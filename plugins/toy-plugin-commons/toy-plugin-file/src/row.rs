...
impl Row {
    pub fn new() -> Row {
        Row::with_capacity(0, 0)
    }

    pub fn with_capacity(buf: usize, column: usize) -> Row {
        Row(Box::new(Inner {
            raw: vec![0; buf],
    ...
    #[inline]
    pub(crate) fn parts(&mut self) -> (&mut Vec<u8>, &mut Vec<usize>) {
...
impl<A: AsRef<[u8]>> FromIterator<A> for Row {
    #[inline]
...
impl<A: AsRef<[u8]>> Extend<A> for Row {
    #[inline]
    ...
    fn next_back(&mut self) -> Option<&'a [u8]> {
        if self.current_col_idx == self.row_length {
            ...
            let start = if self.row_length > 0 {
                self.row.0.edges.get(self.row_length).map(|x| x.start).unwrap_or(0)
            } else {
                0
            };
