use toy_pack_mp::{DecodeError, decoder_from_slice, DecoderOps, marker::Marker};
...
decode_pass!(decode_u16, decode_u16, &[0xcd, 0xff, 0xff], u16::max_value());
decode_pass!(decode_u16_endian, decode_u16, &[0xcd, 0x00, 0x01], 1);

decode_pass!(decode_u32, decode_u32, &[0xce, 0xff, 0xff, 0xff, 0xff], u32::max_value());
decode_pass!(decode_u32_endian, decode_u32, &[0xce, 0x00, 0x00, 0x00, 0x01], 1);

decode_pass!(decode_u64, decode_u64, &[0xcf, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff], u64::max_value());
decode_pass!(decode_u64_endian, decode_u64, &[0xcf, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01], 1);

decode_pass!(decode_i8, decode_i8, &[0xd0, 0x7f], i8::max_value());

decode_pass!(decode_i16, decode_i16, &[0xd1, 0x7f, 0xff], i16::max_value());
decode_pass!(decode_i16_endian, decode_i16, &[0xd1, 0x00, 0x01], 1);

decode_pass!(decode_i32, decode_i32, &[0xd2, 0x7f, 0xff, 0xff, 0xff], i32::max_value());
decode_pass!(decode_i32_endian, decode_i32, &[0xd2, 0x00, 0x00, 0x00, 0x01], 1);

decode_pass!(decode_i64, decode_i64, &[0xd3, 0x7f, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff], i64::max_value());
decode_pass!(decode_i64_endian, decode_i64, &[0xd3, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01], 1);
