...
impl<'a, W> SerializeSeqOps for SerializeCompound<'a, W> where W: Writer {
    type Ok = ();
...
impl<'a, W> SerializeMapOps for SerializeCompound<'a, W> where W: Writer {
    type Ok = ();
    type Error = EncodeError;
...
impl<'a, W> SerializeStructOps for SerializeCompound<'a, W> where W: Writer {
    type Ok = ();
    type Error = EncodeError;

    #[inline]
    fn field<T: ?Sized>(&mut self, _key: &'static str, value: &T) -> Result<(), Self::Error> where T: Serializable {
        value.serialize(&mut *self.ser)
    }
