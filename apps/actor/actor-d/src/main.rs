...
use toy::actor::exporters::{ToyExporter, TracingExporter};
...
impl ActorConfig for Config {
    ...
    fn cert_path(&self) -> String {
        std::env::var("TOY_ACTOR_API_TLS_CERT_PATH").expect("config not found.")
    }

    fn key_path(&self) -> String {
        std::env::var("TOY_ACTOR_API_TLS_KEY_PATH").expect("config not found.")
    }

    fn pub_path(&self) -> String {
        std::env::var("TOY_ACTOR_API_TLS_PUB_PATH").expect("config not found.")
    }
...
fn go() -> Result<(), Error> {
    ...
    match &opts.c {
        Command::Local(c) => {
            ...
            let (sv, _, _) = toy::actor::local(ExecutorFactory, app, config);
        ...
        Command::Subscribe(c) => {
            ...
            let (sv, _, _) =
                toy::actor::subscribe(&c.name, ExecutorFactory, app, api_client, addr, config);
            log_start(&opts, tracing_addr);
...
fn log_start(opts: &Opts, tracing_addr: SocketAddr) {
    tracing::info!("start actor for:{:?}", opts);
