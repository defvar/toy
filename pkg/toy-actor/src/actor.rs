...
use crate::{ActorConfig, Response, RunTaskResponse, TaskResponse};
use chrono::Utc;
...
use toy_api_client::client::{ActorClient, ServiceClient, TaskClient};
...
pub fn local<TF, P, Config>(
    ...
    Actor<TF, P, NoopApiClient, Config>,
    Outgoing<Request>,
    Incoming<()>,
)
where
    ...
    P: Registry + 'static,
    Config: ActorConfig + Clone + Send + Sync + 'static,
{
    Actor::new("local-actor", factory, app, None, None, config)
}
...
pub fn subscribe<S, TF, P, C, Config>(
    ...
    config: Config,
) -> (Actor<TF, P, C, Config>, Outgoing<Request>, Incoming<()>)
where
    S: Into<String>,
    TF: TaskExecutorFactory + Send + 'static,
    P: Registry + 'static,
    ...
    Config: ActorConfig + Clone + Send + Sync + 'static,
{
    Actor::new(name, factory, app, Some(client), Some(addr.into()), config)
}

pub struct Actor<TF, P, C, Config> {
    _factory: TF,
    app: Arc<App<P>>,
    /// receive any request.
    rx: Incoming<Request>,
...
impl<TF, P, C, Config> Actor<TF, P, C, Config>
where
    TF: TaskExecutorFactory + Send + 'static,
    P: Registry + 'static,
    C: ApiClient + Clone + Send + Sync + 'static,
    Config: ActorConfig + Clone + Send + Sync + 'static,
{
    fn new<S: Into<String>>(
        ...
        (
            Actor {
                ...
                rx: rx_req,
                tx_shutdown,
                ctx: ActorContext::with(name, addr, client, tx_req.clone(), schemas),
                config,
            },
            tx_req,
            rx_shutdown,
    ...
    {
        tracing::info!(name=?self.ctx.name(), "oneshot actor");
    ...
    pub async fn run(mut self) -> Result<(), ()> {
        tracing::info!(name=?self.ctx.name(), "start actor");
        ...
        tracing::info!(name=?self.ctx.name(), "shutdown actor.");
        if !self.tx_shutdown.is_closed() {
            ...
            if let Err(e) = self.tx_shutdown.send_ok(()).await {
                tracing::error!(name=?self.ctx.name(), err = %e, "an error occured; actor when shutdown.")
            }
        }
    ...
    async fn register(&self, schemas: Vec<ServiceSchema>) -> Result<(), ()> {
        ...
        let sv = toy_api::actors::Actor::new(
        ...
        if let Err(e) = c
            ...
            .await
        {
            tracing::error!(name=?self.ctx.name(), err = %e, "an error occured; actor when start up.");
            return Err(());
        }
        ...
        for spec in specs {
            ...
            if let Err(e) = c
                ...
                .await
            {
                tracing::error!(name=?self.ctx.name(), err = %e, service_type = %key, "an error occured; actor when register service.");
                return Err(());
            }
        }
...
impl<TF, P, C, Config> std::fmt::Debug for Actor<TF, P, C, Config> {
