...
use crate::exporters::actor_metrics;
use crate::http::Status;
use crate::{ActorError, Request, RunTaskResponse, TaskResponse};
use chrono::Utc;
...
pub async fn status<C>(State(ctx): State<ActorContext<C>>) -> Result<impl IntoResponse, ActorError>
where
    C: ApiClient + Clone + Send + Sync + 'static,
{
    ...
    match toy_pack_json::pack_to_string(&st) {
        ...
        Err(e) => {
            tracing::error!(err = %e);
            Err(ActorError::error(""))
        }
    }
}

pub async fn services<C>(
    State(ctx): State<ActorContext<C>>,
...
pub async fn tasks_post<C>(
    State(mut ctx): State<ActorContext<C>>,
    ...
    C: ApiClient + Clone + Send + Sync + 'static,
{
    ...
    let g = Graph::from(v).map_err(|e| Into::<ActorError>::into(e))?;
    tracing::debug!("{:?}", g);
    ...
    match TaskId::parse_str(&key) {
        Ok(id) => {
            ...
            match rx.recv().await {
                ...
                _ => Err(ActorError::not_found(&key)),
            }
        }
        Err(_) => Err(ActorError::task_id_invalid_format(key)),
    }
}
...
pub async fn event_buffers<C>(
    State(_ctx): State<ActorContext<C>>,
    Query(opt): Query<PostOption>,
) -> Result<impl IntoResponse, ActorError>
where
    C: ApiClient + Clone + Send + Sync + 'static,
    ...
    C: ApiClient + Clone + Send + Sync + 'static,
{
    let format = opt.format();
    let now = Utc::now();
    let r = actor_metrics(now, ctx.name(), metrics::context::metrics()).await;
    Ok(toy_api_http_common::reply::into_response(
        &r,
        format,
        opt.indent(),
